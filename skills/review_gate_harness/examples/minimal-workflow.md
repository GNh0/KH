# Review Gate Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when a UAF workflow needs structured pre-landing review gates, actionable findings, or reviewer status normalization.

The agent must decide whether `review-gate-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
