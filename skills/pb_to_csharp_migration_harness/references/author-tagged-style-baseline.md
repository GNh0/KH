...
- For generated SPs, procedure parameters are caller/C# inputs only. Derived helper/calculation values must be local `DECLARE` variables assigned with `SET`, not procedure parameters. When matched C# `DbParameter` evidence exists, do not add SP parameters outside that caller set.
- Derived date values such as `@YYYY`, `@MM`, `@BASYYYY`, or `@LASTDT` must also be local variables, not generated procedure parameters.
- Do not generate `IF ISNULL(@GIJUNDT, ...)`, `IF ISNULL(@YYYY, ...)`, `IF ISNULL(@MM, ...)`, `IF ISNULL(@BASYYYY, ...)`, or `IF ISNULL(@LASTDT, ...)` date defaulting blocks. Derived SP variables use local `DECLARE` plus `SET` unless verified same-procedure evidence proves an exception.
