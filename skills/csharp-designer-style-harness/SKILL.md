...
이관·미완성 화면 작업에서는 원본의 업무 동작과 대상 프로젝트의 구현 방식을 구분한다. 이미 만든 화면·조회·이벤트 흐름을 바탕으로 빠진 기능을 연결한다. 작성 전에 해당 이벤트의 현재 상태와 실제 target helper를 확인하고 [데이터와 이벤트 계약](references/data-flow.md)에 따라 새 검증·재조회·wrapper·행 키 대입의 필요성을 판단한다. 비교 화면의 이벤트 본문을 역할 확인 없이 복사하지 않는다.
...
검사의 `status=passed`는 실행한 정적 검사에 오류가 없다는 뜻이다. `review_status`, `issues`, `not_checked`를 함께 읽고 현재 요구와 소스를 대조한다. 스킬을 읽었거나 검사만 통과한 상태를 프로젝트 스타일 준수 완료로 보고하지 않는다.
# 제공 조각의 한계
