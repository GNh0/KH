...
def _failure_diagnostic(output: str) -> str | None:
    patterns = (
        r"(?im)^\s*(?:error|failed|failure|exception)\b[^\r\n]*",
        r"(?im)^.*\b(?:export|PBORCA\w*|sessionopen|librarydirectory|libraryentryexport|license)\b[^\r\n]*(?:failed|failure|error|not found|denied|expired|=\s*-\d+)[^\r\n]*",
        r"(?im)^.*(?:오류 발생|내보내기 실패|추출 실패|라이선스 오류)[^\r\n]*",
    )
    for pattern in patterns:
        match = re.search(pattern, output)
        if match:
            return match[0].strip()[:500]
    return None


def _export_files(directory: Path) -> dict[str, tuple[int, int, str]]:
    exports = {}
    if not directory.is_dir():
        return exports
    for path in directory.rglob('*'):
        if path.is_file() and path.suffix.lower() in {'.srw', '.srd', '.sru', '.srf', '.sra', '.srs', '.srm', '.srp'}:
            ...
            for encoding in encodings:
                try:
                    ...
                    continue
            if re.search(r'(?im)^\s*(?:\$PBExportHeader\$|global\s+(?:type|function|subroutine)\b|forward\b|release\s+\d+\s*;)', text):
                stat = path.stat()
                exports[str(path.resolve())] = (stat.st_size, stat.st_mtime_ns, hashlib.sha256(data).hexdigest())
    return exports
...
class PbOrcaRuntime:
    ...
    def probe(self, request: OrcaRequest) -> OrcaCapabilityDecision:
        ...
        elif csc_path is not None and request.compile_helper:
            helper_mode = "compile_x86"
        elif csc_path is not None:
            return self._fallback(
                request, reason_code="helper_compilation_needed",
                message="A usable export helper is absent. Use an existing exported source or select compile_helper when creating the helper is necessary for this extraction.",
                config=config, runtime_dlls=tuple(runtime_dlls), csc_path=csc_path,
            )
        else:
            return self._fallback(
                request,
    ...
    def _build_command(
        ...
        if decision.csc_path is not None and decision.helper_mode == "compile_x86":
    ...
    def _execute(
        ...
        *,
        ...
        if exit_code != 0:
            ...
            status, reason, message = "failed", "orca_failure_diagnostic", "The child returned zero but reported an export/runtime failure: " + diagnostic
        elif request.action != "list" and not fresh:
            status, reason, message = "incomplete", "export_output_unconfirmed", "No new or changed recognizable textual PB export was found; a zero exit code is insufficient."
        elif request.action == "list" and not stdout.strip():
            status, reason, message = "incomplete", "object_list_unconfirmed", "The command returned no object inventory."
        else:
            status, reason, message = "completed", "conversion_completed", "Export process and fresh textual output checked; whole application behavior remains unverified."
        return OrcaConversionResult(
            status=status,
            reason_code=reason,
            message=message,
...
def _build_request(arguments: argparse.Namespace) -> OrcaRequest:
    return OrcaRequest(
        ...
        compile_helper=arguments.compile_helper,
    )
