...
A user says: "Build a SaaS dashboard, verify the local browser screen, and prepare the PR." The host has three available providers:

- `kh`: capabilities `workflow_control`, `memory_goal_resume`, `domain_orchestration`, `tdd_review`.
- `visual-checker`: capabilities `browser_qa`, `screenshot`.
...
2. Normalize capabilities:
   ...
   - Browser-style visual QA and screenshots.
...
10. Hand browser and repo work only to their assistant scopes.
  ...
  "assistants": [
    {
      ...
      "scope": "visual QA, screenshots, and local web verification"
    },
    {
...
- Browser QA is required but no provider is available and no fallback is recorded.
