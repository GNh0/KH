...
- `workspace_strategy`: `host-worktree`, `project-local-worktree`, `isolated-branch`, or `current-checkout` with the in-place exception reason when isolation is not used
...
- `report_fields`: `status`, `changed_files`, `commands_run`, `evidence`, `commit_sha`, `workspace_strategy`, `kh_front_door_evidence`, `selected_harness_evidence`, `token_optimizer_status`, `token_optimizer_provider`, `nested_subagents_available`, `harness_usage`, `concerns`
