...
- Standard packet inputs from `references/standard-task-packets.md`: implementer, spec-reviewer, and code-quality-reviewer roles; workspace; base/head SHA; plan section; owned/forbidden files; checks; report fields; and per-task commit policy.
...
3. Create a compact task packet per implementer and keep it self-contained; use `references/standard-task-packets.md` as the canonical prompt packet shape.
...
6. For task-plan development work, update `.kh/development/<run-id>/state/progress.json` after implementer, review, fix, re-review, and commit events.
...
- Development progress state path and the task loop stage last updated.
