...
Use these packet shapes when KH coordinates host subagents for development work. Keep each packet self-contained and bounded. Select `token-optimizer` only for a large reducible packet, command log, transcript, or an explicit telemetry request.
...
- `kh_front_door_evidence`: required only when governed routing or an explicit routing audit actually selected the front door; omit it for ordinary direct-domain dispatch
...
- `token_optimizer_status`: required only after `token-optimizer` was actually selected for a large reducible packet, command output, transcript, or explicit telemetry request; otherwise omit it
- `token_optimizer_provider`: `kh`, `rtk`, or `hybrid` only when `token-optimizer` was selected; use `token_optimizer_status=passthrough` when selected content must remain exact
...
- `report_fields`: always include `status`, `changed_files`, `commands_run`, `evidence`, `commit_sha`, `workspace_assignment`, `workspace_strategy`, `workspace_decision_source`, `worktree_isolation_evidence`, `selected_harness_evidence`, `nested_subagents_available`, `final_user_language_policy`, `harness_usage`, and `concerns`; add front-door and token-optimizer fields only when those gates were selected
...
- Do not accept an implementer report as complete when it omits `workspace_strategy` or selected-harness evidence. Require `kh_front_door_evidence` or token-optimizer fields only when the corresponding gate was actually selected.
...
- Treat `token_optimizer_status=considered_not_needed` as valid only after the skill was selected and the report states why the anticipated large payload stayed small enough for no compression.
- Update progress after each implementer report, review result, fix, re-review, and commit.
- Preserve `workspace_strategy`, `task_status`, `review_status`, `commit_sha`, and `next_task` in the final response; add `token_optimizer_status` only when that gate was selected.
...
- Decide whether `token-optimizer` applies before a large reducible task packet, command output, or subagent transcript. When it does not apply, omit synthetic token status; when it does, never compress away reviewer severity, failing command output, or commit evidence.
