...
- For XML-based SAVE work, build a field-provenance matrix before SQL: target field, PB/UI source, ownership class, required/editable state, XML membership, INSERT membership, UPDATE membership, fixed literal or DB/server ownership, and evidence reference. Ownership classes are mutually exclusive.
- Omit nullable target fields that the PB screen neither supplies nor uses. Omit DB-default fields from XML and generated DML. Do not use `ISNULL`, `NULLIF`, empty strings, or zero merely to populate those columns.
- Treat a PB fixed value as a direct SQL literal only when PB evidence proves it. Treat an editable control's initial value as user input, not a fixed literal. Required editable values must fail fast before the first target-table write with the target `IF` / `BEGIN` / `RAISERROR` / `RETURN` pattern. Put only required textual fields that reject `''` in `required_nonblank_fields`; do not compare numeric/date fields to empty strings.
- Declare exact INSERT and UPDATE field projections independently and validate them with `verify_pb_migration_save_field_contract`; the target DML may contain only payload, technical, PB-fixed, or server-derived fields authorized by that contract.
