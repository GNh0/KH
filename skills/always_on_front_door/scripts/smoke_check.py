...
def resolve_target(repo_root: Path, ref: str) -> dict[str, str]:
    ...
    tests_packaged = tests_dir.is_dir() and any(tests_dir.glob("test_*.py"))
    if ref.startswith("tests.") and not tests_packaged:
        return {
            "ref": ref,
            "status": "packaged_test_reference",
            "detail": "tests are not packaged in the slim runtime branch; run from the full source checkout",
        }
...
def main() -> int:
    ...
    if repo_root is not None:
        for target in targets:
            ...
            if result["status"] not in {"resolved", "packaged_test_reference"}:
                issues.append({
