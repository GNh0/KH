...
- 45 packaged skills/harnesses with support files, smoke checks, and runnable demos.
...
- `csharp-designer-style-harness` for generic, PB-independent verification of one exact hash-bound C# WinForms/DevExpress/KoneLib source and Designer pair.
- The `sql-formatting` provider supplies the host-executed formatting procedure; the separate `sql-formatting-style-harness` verifier checks preservation and style after the candidate is generated.
...
- Git-backed implementation should prefer an isolated workspace by default. Before starting Git, KH uses a filesystem-only `.git`/worktree-pointer gate; non-Git targets skip Git and GitHub branch-finishing commands without retrying them. Use host worktrees when the host provides them; otherwise use project-local `.worktrees/<task>` or an isolated branch. Git worktrees should live under `.worktrees/` when KH creates project-local isolation. Use the current checkout only for non-Git work, docs-only edits, a single-file small patch, or explicit in-place user instruction, and report `workspace_strategy` in the final status.
- Multi-task development runs should keep machine-readable progress at `.kh/development/<run-id>/state/progress.json`. The progress state records active task, RED/GREEN evidence, spec/code-quality review status, fix/re-review status, commit SHA, next task, `workspace_strategy`, `token_optimizer_status`, and `token_optimizer_status_reason`.
...
- `src.orchestration.runtime_token_optimizer.optimize_workflow_task_results(...)` attaches runtime token optimization records to task command outputs and subagent transcripts, reports workflow-level optimizer-local payload savings with explicit `billing_tokens_available` metadata, and emits RTK-style command-family statistics while preserving failing command context and lifecycle evidence.
...
- `src.orchestration.token_optimizer_provider.resolve_token_optimizer_provider(...)` records `token_optimizer_provider`: `kh`, `rtk`, or `hybrid`. RTK is optional; hybrid falls back to KH and exact source-of-truth text is reported with `token_optimizer_status=passthrough`.
...
- `src.orchestration.progress_panel.render_progress_panel(...)` gives long task-plan work a visible status panel with task status, review status, token optimizer status and reason, commit SHA, and next task.
...
Skill usage is separated from skill inspection. For example, reading `token_optimizer/SKILL.md` or quoting `token_optimizer_status` does not count as token optimizer usage; the postmortem requires runtime evidence such as `src.skills.token_optimizer`, command-output summarization, RTK-style payload telemetry such as `estimated_payload_tokens_saved`/`where_saved`, host evidence such as `host_actual_token_evidence` when Codex session `token_count` or GoalState `tokensUsed` is available, or explicit passthrough evidence. If usage is blocked or not needed, postmortem and progress outputs include `token_optimizer_status_reason`. KH does not label payload estimates as provider billing tokens.
