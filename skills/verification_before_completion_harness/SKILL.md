...
2. Run the filesystem-only Git workspace gate. For non-Git targets, do not invoke Git and limit the integration claim to a local non-Git result.
3. Read the active `GoalState`, development progress state, changed files, and review/QA status.
4. Select the smallest fresh verification command that proves the changed behavior and the broader command that protects integration.
5. Run or require the verification immediately before the completion claim; stale output from an earlier step is not enough.
6. Preserve command, exit code, pass/fail, sandbox retry reason, and important failure text through `command-output-harness` or `token-optimizer` when output is large.
7. Block completion when verification is missing, failed, stale, narrower than the claim, or replaced by a weaker check without disclosure.
8. Update GoalState evidence, progress state, and final report fields with `verification_status`, `review_status`, `commit_sha`, `workspace_strategy`, and `next_task`.
9. If final verification cannot run, report `blocked` or `done_with_residual_risk` with the exact missing tool, permission, environment, or command.
...
- Do not run Git completion checks for a target that failed the filesystem-only Git workspace gate.
