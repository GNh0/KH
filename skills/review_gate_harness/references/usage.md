# Review Gate Harness Usage Reference

This reference expands the portable operating contract for `review-gate-harness`. Read it when the task is real work, when deciding whether this skill applies, or when a review needs evidence beyond the concise `SKILL.md`.

## When to use

Use when a UAF workflow needs structured pre-landing review gates, actionable findings, or reviewer status normalization.

Context summary: This is a UAF-native review harness for structured review workflow patterns. It must not call or require an external review runtime at execution time.
...
A valid use of `review-gate-harness` must leave enough evidence for another agent to answer: why this skill applied, what ran or was applied, what changed, and what still needs attention.
