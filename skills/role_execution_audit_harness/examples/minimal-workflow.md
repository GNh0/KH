# Role Execution Audit Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when UAF role DAG execution must prove roles actually ran, produced artifacts, and included parallel waves.

The agent must decide whether `role-execution-audit-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
