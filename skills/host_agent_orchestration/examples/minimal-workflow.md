# Host Agent Orchestration Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when designing portable UAF host agent loops, subagent delegation, tool permissions, hooks, or observability across Codex, Antigravity-style, Claude Code, or local runtimes.

The agent must decide whether `host-agent-orchestration` applies, run or apply it according to its execution level, and leave auditable evidence.
