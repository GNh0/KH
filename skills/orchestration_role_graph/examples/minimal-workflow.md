# Orchestration Role Graph Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when inspecting or extending the default UAF orchestration roles such as CEO, advisor, architect, controller, implementer, reviewers, QA, security, and release.

The agent must decide whether `orchestration-role-graph` applies, run or apply it according to its execution level, and leave auditable evidence.
...
- `skill`: `orchestration-role-graph`.
