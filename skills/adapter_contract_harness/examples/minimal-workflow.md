# Adapter Contract Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when adding or normalizing dispatch adapters for Codex, Antigravity-style agents, Claude Code, local workers, or other agent runtimes.

The agent must decide whether `adapter-contract-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
