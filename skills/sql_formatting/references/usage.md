...
5. Record a complete alias plan for every multi-source formatted scope, even when aliases are preserved, and for every single-source formatted scope whose alias changes. Role-family membership must be reviewer-approved semantic input, not an inference from repeated table names alone. Assign role-family letters by first SQL appearance. After all non-alias formatting is complete, call `bind_sql_alias_role_plan(host_candidate_sql, plan)` against the exact pre-alias candidate that will be passed to `prepare_candidate.py`, then apply only that bound plan with `apply_sql_alias_role_plan(...)`. Any pre-alias candidate edit invalidates the binding and requires a newly reviewed plan. Run `normalize_sql_join_layout(...)` before verification so aliases and join predicate indentation are not manually retyped.
...
- If verification fails on a presentation-only issue, revise the same candidate from the structured issue evidence, rerun the deterministic normalizers, verifier, and binder, and keep the failed candidate hidden. Ask the user only when the remaining conflict is semantic or the business-role grouping is genuinely ambiguous.
