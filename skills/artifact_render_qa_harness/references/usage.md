# Artifact Render QA Harness Usage Reference

This reference expands the portable operating contract for `artifact-render-qa-harness`. Read it when the task is real work, when deciding whether this skill applies, or when a review needs evidence beyond the concise `SKILL.md`.
...
2. Read this reference before performing non-trivial work with `artifact-render-qa-harness`.
...
A valid use of `artifact-render-qa-harness` must leave enough evidence for another agent to answer: why this skill applied, what ran or was applied, what changed, and what still needs attention.
