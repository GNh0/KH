...
- Before generating an XML-based SAVE procedure, build one evidence-backed field ownership contract. Classify each target field exactly once as editable payload, technical row/key data, PB-fixed literal, database default, server-derived value, or nullable unused field. Record exact INSERT and UPDATE projections separately.
- Serialize only editable payload and technical row/key fields. Omit nullable fields unused by the PB screen and fields owned by database defaults; do not force them into XML or DML with `''`, `0`, `ISNULL`, or `NULLIF`.
- Emit PB fixed constants as their authoritative SQL literals. A user-editable control's initial value is not a fixed SP default: validate required input with the target `IF` / `BEGIN` / `RAISERROR` / `RETURN` pattern before the first target-table write, then save the exact supplied value. Declare textual fields that also reject `''` separately as `required_nonblank_fields`; numeric/date required fields need the type-appropriate null guard without an invented empty-string comparison.
- Run `verify_pb_migration_save_field_contract` for every XML-based generated SAVE procedure and pass the same `save_field_contract` through SP generation, SQL binding, and orchestration validation. Missing ownership or projection evidence blocks release.
