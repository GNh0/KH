# UAF Skill Catalog Usage Reference

This reference expands the portable operating contract for `skill-catalog`. Read it when the task is real work, when deciding whether this skill applies, or when a review needs evidence beyond the concise `SKILL.md`.

## When to use

Use when listing, reading, or adding packaged UAF skills and harnesses in this repository.

Context summary: UAF skills are packaged inside this repository under `skills/<skill-name>/SKILL.md`. Do not depend on installed external skill folders at runtime.
...
2. Read this reference before performing non-trivial work with `skill-catalog`.
...
A valid use of `skill-catalog` must leave enough evidence for another agent to answer: why this skill applied, what ran or was applied, what changed, and what still needs attention.
