...
- Preferred family evidence is `source_bound_role_rationale` with exactly `kind`, a `query://` or `sql://` `source`, exact `role_names`, and a non-empty `rationale`. `bind_sql_alias_role_plan(...)` binds it to the exact SQL hash, scope id, and declaration fingerprint. It is host/caller-declared, not reviewer-approved, and not externally authenticated. It may pass formatting only because complete token/predicate preservation and every other release gate still pass.
- Structured `reviewer_approved_business_role` objects and compact `review://<review-id>/<declared-role-names>-roles` strings remain compatibility forms. Do not self-author or fabricate them. Metadata reports the basis modes separately from external authentication status. When the family decision is ambiguous, obtain an independent review or user confirmation instead of presenting structural validation as semantic proof.
