# Health Check Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when a UAF workflow needs a code quality dashboard, test health summary, static checks, or release readiness score.

The agent must decide whether `health-check-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
