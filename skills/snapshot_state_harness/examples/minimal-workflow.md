# Snapshot State Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when UAF workflows need rollback points, compressed file snapshots, state restore, or safe checkpoint metadata before generated code changes.

The agent must decide whether `snapshot-state-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
