...
5. Complete alias plans for every multi-source formatted scope, plus every single-source formatted scope whose alias changes. The structural first source is `A`. Walk source declarations in SQL order: the first new support role family is `B`, the next is `C`, then `D`; when one family has multiple members, number every member from `1` (`B1`, `B2`, ...). Prefer a host/caller-declared `source_bound_role_rationale` tied to the current SQL; use reviewer-approved evidence only when it genuinely exists. After all non-alias formatting, call `bind_sql_alias_role_plan(host_candidate_sql, plan)` once against the exact pre-alias candidate passed to `prepare_candidate.py`; it records that candidate's SHA-256 and ordered declaration fingerprint for every planned scope. Apply only that bound plan with `apply_sql_alias_role_plan(...)`. If the pre-alias candidate changes, discard and regenerate the plan rather than rebinding a stale plan.
