...
Skill usage is separated from skill inspection. For example, reading `token_optimizer/SKILL.md` or quoting `token_optimizer_status` does not count as token optimizer usage; the postmortem requires runtime evidence such as `src.skills.token_optimizer`, command-output summarization, optimizer-local payload telemetry such as `actual_tokens_saved`/`actual_usage_scope`, or explicit passthrough evidence. If usage is blocked or not needed, postmortem and progress outputs include `token_optimizer_status_reason`. KH does not label those payload counts as provider billing tokens unless the host exposes billing telemetry.
