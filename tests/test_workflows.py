...
class WorkflowDispatchTests(unittest.TestCase):
    ...
    def test_workflow_usability_auto_writes_progress_panel_and_compound_handoff(self):
        original_url = os.environ.pop("AG_WEBHOOK_URL", None)
        metadata = build_default_role_metadata()
        metadata["workflow_usability_auto"] = True
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                usability = result.metadata["workflow_usability"]
                progress_path = Path(usability["progress_path"])
                compound_path = Path(usability["compound"]["paths"]["compound_handoff"])
                self.assertTrue(progress_path.exists())
                self.assertTrue(compound_path.exists())
        finally:
            if original_url is not None:
                os.environ["AG_WEBHOOK_URL"] = original_url
        ...
        self.assertIn("scenario_candidates", usability["compound"])
