# Harness Evaluator Skill Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when Python code needs isolated syntax, runtime, or module verification before presenting results.

The agent must decide whether `harness-evaluator` applies, run or apply it according to its execution level, and leave auditable evidence.
