# Traceability Matrix Harness Minimal Workflow Example

## Scenario

A host agent receives a task where this trigger is relevant: Use when UAF workflows need requirements, deliverables, evidence keys, and review gates mapped without exporting internal harness spreadsheets.

The agent must decide whether `traceability-matrix-harness` applies, run or apply it according to its execution level, and leave auditable evidence.
...
- `skill`: `traceability-matrix-harness`.
