...
KH 3.0.0 provides ten focused skills and optional local checks for SQL, C#/Designer and PowerBuilder work. [한국어 및 상세 사용법](README.ko.md)
...
Results distinguish errors, warnings and incomplete input, and list both `checked` and `not_checked`. No DB, build, API key, server or package installation is needed. Static source/container checks do not establish runtime behavior or visual quality. `sql --normalize-layout` only normalizes supported JOIN/EXISTS layout to stdout.

Run `python -B -m unittest discover -s tests/domain` from this repository for local tests. See [documentation and migration notes](docs/README.md).
